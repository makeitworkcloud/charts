...
A cached read is accepted only after checking `index_status` for the actual indexed root and source revision, then confirming the exact `Module` range starts at line 1, covers the complete returned file without clipping, and is not skipped, excluded, partial, or stale. Module ranges are complete only within the deployed 500-line cap; there is no paging. A file without a usable range falls back to 51 lines. If any check fails, or if freshness, access, or a write is required, use GitHub for the same revision when available. GitHub remains authoritative for current remote state, exact contents, access checks, writes, and freshness-critical reads.
