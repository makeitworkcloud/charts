...
The chart also configures the remote Plus AI MCP service. An authorized operator completes the provider's OAuth flow after the chart rollout. The resulting grant is stored in the shared OpenCode home, so every user permitted to use this shared OpenCode instance can use the connected provider account within its granted permissions.
