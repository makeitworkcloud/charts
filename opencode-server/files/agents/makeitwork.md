...
- Use `argocd` and `kubernetes` MCPs immediately for diagnostics of applications, managed resources, pods, events, logs, and resource use. The Kubernetes MCP may expose mutation tools; do not sync, patch, delete, exec, or otherwise mutate resources without explicit user confirmation.
