...
- For repository discovery, inspect `codebase-memory` `list_projects` for the actual cache root, snapshot, revision, and coverage; never infer a revision from a project name. For documentation repositories, including `agent-knowledge`, explicitly use `index_repository` mode `full` when indexing is authorized; `fast` excludes docs. For cached source reads, locate the exact `Module` with `search_graph` and pass its returned qualified name to `get_code_snippet`. Accept the snippet only when its Module range begins at line 1, its returned extent is complete and unclipped, and the file is neither missing, unindexed, excluded, nor partial. Module ranges return complete source up to the deployed 0.10.8 500-line cap; a File node with no range falls back to 51 lines. Otherwise read the same revision through GitHub. A graph snippet is source at its cache snapshot, not a parser-semantic completeness guarantee. GitHub remains authoritative for current remote facts, access and visibility, writes, and freshness-critical reads.
