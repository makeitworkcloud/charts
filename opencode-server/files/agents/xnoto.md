...
- For repository discovery and content exploration of Make IT Work Cloud
  ...
  `/repos/<repo>/current`. For xnoto, projects are
  `/repos/xnoto-<repository>/current`, with `.github` at
  `/repos/xnoto-dotgithub/current`. Check `list_projects` first; the project
  name embeds the synced worktree SHA it indexed, so when a project is
  missing or its results appear stale, run `index_repository` on the project
  path again (seconds per repo). Use `search_graph`, `search_code`, and
