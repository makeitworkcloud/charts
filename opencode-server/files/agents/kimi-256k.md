...
If the cache is missing, unindexed, stale, incomplete, has an unknown, unverified, or mismatched root, root hash, or HEAD, or the read is partial, skipped, excluded, clipped, or otherwise fails a range or coverage check, log the specific reason and use GitHub MCP `get_file_contents` with `sha=<verified snapshot SHA>` when available. If that snapshot is unavailable, label the current GitHub content as a different snapshot. Branch pull-request content cannot match the default-branch cache snapshot; read it on GitHub at the branch SHA. If the root disappears or changes mid-task, discard the affected reads and fall back. GitHub MCP is authoritative for current remote state, freshness, access, and writes; do not use a stale SHA for those checks. Do not use `/repos` native reads, run `index_repository`, or change the backend or mount.
