...
- For cached source reads, locate the exact `Module` with `search_graph` and pass its returned qualified name to `get_code_snippet`. Accept the cached source only when the project's actual root revision is known from index metadata, the Module range begins at line 1, the returned extent is complete and unclipped, and the file is neither missing, unindexed, excluded, nor partial. Module ranges return complete source up to the deployed 0.10.8 500-line cap; a File node with no range falls back to 51 lines. Otherwise, read the same revision through GitHub when possible. For documentation repositories, including `agent-knowledge`, explicitly use `index_repository` mode `full` when indexing is authorized; `fast` excludes docs. Cached KB startup docs may provide startup context, but GitHub remains authoritative for access checks and freshness-critical reads.
