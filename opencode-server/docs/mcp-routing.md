...
the generated `kustomize-cluster` chart-pin PR are separate stages. After the
pin is selected and reconciled, verify a fresh OpenCode session discovers the
direct integrations and performs bounded read-only representative calls. Do
not infer runtime reachability from chart CI.
